...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        int[] arraySizes = {125000, 250000, 500000, 1000000, 2000000};
        int[] cutoffs = {250, 500, 1000, 2000, 4000, 8000, 16000, 32000, 64000, 128000, 256000, 512000};
        ...
        for (int arraySize : arraySizes) {
            ...
            for (int threadCount : threadCounts) {
                ...
                for (int cutoff : cutoffs) {
                    ...
                    double rate = (double) cutoff / arraySize;
                    System.out.println("Array Size: " + arraySize + ", Cutoff: " + cutoff + ", Threads: " + threadCount + ", Avg Time: " + avgTime + " ms" + String.format("Cutoff/Size: %.5f", rate));
                    results.add(arraySize + "," + cutoff + "," + threadCount + "," + avgTime + "," + String.format("%.5f", rate));
                }
            }
        }

        try (BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("./src/result.csv")))) {
            bw.write("ArraySize,Cutoff,Threads,AvgTime(ms),cutoff/size\n");
